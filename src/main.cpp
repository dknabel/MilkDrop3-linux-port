  ...
  void shutdown() noexcept {
    try {
      ...
      try {
        ...
        std::cerr << "Exception during visualizer shutdown: " << e.what() << "\n";
      }
      ...
      try {
        ...
        std::cerr << "Exception during display shutdown: " << e.what() << "\n";
      }

      try {
        graphics_.reset();
      } catch (const std::exception& e) {
        std::cerr << "Exception during graphics shutdown: " << e.what() << "\n";
      }

      try {
        window_.reset();
      } catch (const std::exception& e) {
        std::cerr << "Exception during window shutdown: " << e.what() << "\n";
      }

      try {
        audio_.reset();
      } catch (const std::exception& e) {
        std::cerr << "Exception during audio shutdown: " << e.what() << "\n";
      }
      ...
      try {
        ...
        std::cerr << "Exception during config save: " << e.what() << "\n";
      }
    } catch (...) {
    ...
    try {
      ...
      while (display_->isRunning()) {
        try {
          ...
          // Clamp deltaTime to reasonable values to prevent numerical issues
          if (deltaTime < 0.0f) deltaTime = 0.0f;
          if (deltaTime > 1.0f) deltaTime = 0.016f;  // Cap at ~60fps equivalent
          ...
          try {
            ...
            std::cerr << "Exception during input handling: " << e.what() << "\n";
          }
          ...
          try {
            ...
            // Continue running despite audio error
          }
          ...
          try {
            ...
            std::cerr << "Exception during display update: " << e.what() << "\n";
            break;
          }
        } catch (const std::exception& e) {
      ...
      std::cerr << "Fatal exception during run: " << e.what() << "\n";
      shutdown();
      return false;
    } catch (...) {
      std::cerr << "Unknown fatal exception during run\n";
      shutdown();
      return false;
    }
  }
...
int main() {
  ...
  try {
    ...
    if (!app.run()) {
      std::cerr << "Application run failed\n";
      exitCode = 1;
    }
